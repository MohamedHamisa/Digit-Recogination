...
while True:
    ret, img = cap.read()
    #img = cv2.flip(img, 1)
    img = img[200:400, 200:400]   #crop the image
...
def draw(event, x, y, flag, params):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if run == True:
            cv2.circle(img, (x,y), 20, (255,255,255), -1)  #thickness =-1

  #if the treger is moving like a circle
        ...
        gray = gray.reshape(1, 784)
        result = np.argmax(model.predict(gray))
        result = 'cnn : {}'.format(result)  #word top of the window
        cv2.putText(img, org=(25,follow), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, text= result, color=(255,0,0), thickness=1)
        follow += 25  #if i moved more than one the result will change and put 25 will put the text below
        #if i pressed the power button down it will be taking the image to blank and reset the value of follow
